...
/// The shared superclass of all the typed buffer subclasses.
mixin TypedDataBufferMixin<E, L extends List<E>> on List<E> {
  static const int _initialLength = 8;

  /// The underlying data buffer.
  abstract L sourceBuffer;
  ...
  L createBuffer(int size);

  L get _buffer => sourceBuffer;

  set _buffer(L newBuffer) {
    sourceBuffer = newBuffer;
  }

  /// Returns a view of [_buffer] as a [TypedData].
  TypedData get sourceTypedBuffer;

  /// The length of the list being built.
  late int _length = _buffer.length;
  ...
  set length(int newLength) {
    if (newLength < _length) {
      var _defltValue = defaultValue;
      for (var i = newLength; i < _length; i++) {
        _buffer[i] = _defltValue;
      }
    } else if (newLength > _buffer.length) {
      L newBuffer;
      if (_buffer.isEmpty) {
        newBuffer = createBuffer(newLength);
      } else {
  ...
  /// the current buffer.
  L _createBiggerBuffer(int? requiredCapacity) {
  ...
  void _setRange(int start, int end, Iterable<E> source, int skipCount) {
    if (source is TypedDataBufferMixin<E, L>) {
  ...
  int get elementSizeInBytes => sourceTypedBuffer.elementSizeInBytes;

  int get lengthInBytes => _length * sourceTypedBuffer.elementSizeInBytes;

  int get offsetInBytes => sourceTypedBuffer.offsetInBytes;
  ...
  ByteBuffer get buffer => sourceTypedBuffer.buffer;
}
